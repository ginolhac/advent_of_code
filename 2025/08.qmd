---
title: "08"
format: html
---

```{r}
#| label: setup
#| include: false
library(tidyverse)
library(tictoc)
library(plotly)
library(R6)
library(dendextend)
library(circlize)
options(scipen = 999)
```

## Part 1


```{r}
input <- "162,817,812
57,618,57
906,360,560
592,479,940
352,342,300
466,668,158
542,29,236
431,825,988
739,650,466
52,470,668
216,146,977
819,987,18
117,168,530
805,96,715
346,949,466
970,615,88
941,993,340
862,61,35
984,92,344
425,690,689"


input <- "input/08"

tibble(e = read_lines(input)) |> 
  separate_wider_delim(cols = e, delim = ",", names_sep = "_") |> 
  mutate(across(everything(), as.numeric)) -> coords
coords
```

```{r}
#| eval: false
plot_ly(coords, x = ~ e_1, y = ~ e_2, z = ~ e_3) |> 
  layout(scene = list(xaxis = list(title = 'X'),
                      yaxis = list(title = 'Y'),
                      zaxis = list(title = 'Z')))
```

```{r}
as.matrix(coords) |> 
  dist() -> dist_dots
#plot(hclust(dist_dots))
mdist <- as.matrix(dist_dots)

tibble(
  row = rep(seq_len(nrow(mdist)), ncol(mdist)), 
  col = rep(seq_len(ncol(mdist)), each = nrow(mdist)), 
  value = c(mdist)) |> 
  filter(row < col) |> 
  slice_min(value, n = 10) -> ten_closests
ten_closests
```

## Quick Union as a R6 Class

```{r}

Qunion <- R6Class(
  "Qunion",
  list(
    id = NULL,
    sz = NULL,
    initialize = function(size = 10L) {
      self$id <- seq_len(size)
      self$sz <- rep(1, size)
    },
    init = function() {
      message("New quick-union of size ", length(self$id))
    },
    size = function() {
      length(self$id)
    },
    print = function(...) {
      cat("Quick-union: \n")
      cat("   i:  ", str_pad(seq_along(self$id), 2L), "\n", sep = " ")
      cat("   id: ", str_pad(self$id, 2L), "\n", sep = " ")
      cat("   sz: ", str_pad(self$sz, 2L), "\n", sep = " ")
      invisible(self)
    },
    find_root = function(p) {
      while (p != self$id[p]) {
        # path compression
        self$id[p] <- self$id[self$id[p]]
        p <- self$id[p]
      }
      p
    },
    connected = function(p, q) {
      self$find_root(p) == self$find_root(q)
    },
    quick_union = function(p, q) {

      if (self$connected(p, q)) {
        #cat("p ", p, " q ", q, " already connected\n")
        return(invisible(self))
      }
      root_p <- self$find_root(p)
      root_q <- self$find_root(q)
      #  Connect the smaller tree to the larger tree
      if (self$sz[root_p] < self$sz[root_q]) {
        self$id[root_p] <-  root_q
        self$sz[root_q] <- self$sz[root_p] + self$sz[root_q]
      } else {
        self$id[root_q] <-  root_p
        self$sz[root_p] <-  self$sz[root_q] + self$sz[root_p]
      }
      # R6 method called for its side-effects (usually modifying the object) should return this:
      invisible(self)
    }
  )
)


tic()
clusters <- Qunion$new(size = flatten_int(select(ten_closests, -value)) |> max())
for (i in seq_along(ten_closests$row)) {
  # open site to connect them
  clusters$quick_union(ten_closests$row[i], ten_closests$col[i])
}
sort(clusters$sz, decreasing = TRUE)[1:3] |> prod()
toc()
```

66912  0.388 sec elapsed


## Part 2

```{r}
as.matrix(coords) |> 
  dist() -> dist_dots

mdist <- as.matrix(dist_dots)

tibble(
  row = rep(seq_len(nrow(mdist)), ncol(mdist)), 
  col = rep(seq_len(ncol(mdist)), each = nrow(mdist)), 
  value = c(mdist)) |> 
  filter(row < col) |> 
  arrange(value, row) -> closests

tic()
clusters2 <- Qunion$new(size = flatten_int(select(closests, -value)) |> max())
for (i in seq_along(closests$row)) {
  # open site to connect them
  clusters2$quick_union(closests$row[i], closests$col[i])
  # the moment we connect everything!
  if (max(clusters2$sz) == clusters2$size()) {
    message(paste("connect ", closests$row[i], closests$col[i], " size max:", max(clusters2$sz)))
    ids <- c(closests$row[i], closests$col[i])
    break
  } 
}
coords[ids, "e_1"]$e_1 |> prod()
toc()
```

`102` and `827` are the two nodes that connect everyone

```{r}
dend <- as.dendrogram(hclust(dist_dots))
res <- 300
ragg::agg_png("d08.png", width = 8 * res, height = 6 * res, res = res)
dend |> 
  set("labels_cex", ifelse(labels(dend) %in% ids, 2, 0.1)) |> 
  color_labels(col = "purple", labels = ids) |> 
  set("by_labels_branches_col", value =  ids, type = "any", TF_values = c("purple", "black")) |> 
  set("by_labels_branches_lwd", value =  ids, type = "any", TF_values = c(5, 1)) |> 
  circlize_dendrogram() 
dev.off()
```

