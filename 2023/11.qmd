...
```{r}
distance_with_expansion <- function(x, y) {
  expansion <- 0L
  expansion <- expansion + sum(abs(galpos[[x]][1]:galpos[[y]][1]) %in% expansion_rows)
  expansion <- expansion + sum(abs(galpos[[x]][2]:galpos[[y]][2]) %in% expansion_cols)
