...
find_shift_gr <- function(query, subject) {
  ...
  to_shit <- setdiff(query, no_shift)
  # which interval for shifting
  ov <- findOverlaps(query, subject, type = "any")
  map(seq_along(ov), \(x) {
    #message("query ", queryHits(ov)[x], " goes on soil ", subjectHits(ov)[x], " shift with ", soil$shift[subjectHits(ov)[x]])
    shift(to_shit[queryHits(ov)[x]], subject$shift[subjectHits(ov)[x]])
  })  -> shifted
...
find_shift_gr(seed_ir, garden_gr$`seed-to-soil`) |> 
