...
find_correspondences <- function(query, intervals) {
  for (rg in intervals) {
    if (query %in% as.numeric(names(rg))) {
...
seeds |> 
  map_int(\(x) find_correspondences(x, garden_map$`seed-to-soil`), ) |> 
  map_int(\(x) find_correspondences(x, garden_map$`soil-to-fertilizer`)) |> 
  map_int(\(x) find_correspondences(x, garden_map$`fertilizer-to-water`)) |> 
  map_int(\(x) find_correspondences(x, garden_map$`water-to-light`)) |> 
  map_int(\(x) find_correspondences(x, garden_map$`light-to-temperature`)) |> 
  map_int(\(x) find_correspondences(x, garden_map$`temperature-to-humidity`)) |> 
  map_int(\(x) find_correspondences(x, garden_map$`humidity-to-location`))
