...
solve_ties <- function(rank, tib) {
  ...
  #   # if ties in max, go to next card
  #   if (length(which(mat[i, ] %in% max(mat[i, ]))) > 1L) {
  #     message("next", length(which(mat[i, ] %in% max(mat[i, ]))))
  #     next
  #   } else {
  #     if (i == 5L) {
  #       message("Tie after 5 cards, rank: ", rank)
  #     }
  #     # if not, return the hands that wins
  #     #return(which.max(mat[i, ]))
  #     return(rank(mat[i, ]) / 1000 + rank)
  #   }
  # }
}
```
