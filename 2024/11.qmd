...
Stone <- R6Class(
  ...
  list(
    ...
    initialize = function(line) {
      ...
      for (i in seq_along(starters)) sethash(self$stones, starters[i], 1L)
    },
    init = function() {
    ...
    blink = function() {
      for (k in self$hashkeys()) {
        #print(paste(k, gethash(self$stones, k)))
        # 0 becomes 1
        if (k == "0") {
          ones <- gethash(self$stones, "1", nomatch = 0L) 
          sethash(self$stones, "1", ones + gethash(self$stones, "0", nomatch = 0L))
        } else {
          ...
          if (str_length(k) %% 2 == 0) {
            ...
            sethash(self$stones, right, gethash(self$stones, right, nomatch = 0L) + 1L)
          } else {
            #print(paste("2024 ", k))
            new_key <- (as.integer(k) * 2024) |> as.character()
            #print(paste("2024: ", k, new_key))
            sethash(self$stones, new_key, gethash(self$stones, new_key, nomatch = 0L) + 1L)
          }
