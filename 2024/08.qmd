...
row_anti <- function(d, r1, r2, dx) {
  # anti left side
  row_anti1 <- ifelse(d, min(r1, r2) - (abs(dx)*1:10), max(r1, r2) + (abs(dx)*1:10))
  print(row_anti1)
  #row_anti1[between(row_anti1, 1, matdim[1])]
  list(row_anti1)
}

col_anti <- function(d, c1, c2, dy) {
  # anti left side
  col_anti1 <- min(c1, c2) - abs(dy)
  list(col_anti1)
}
...
combn(A$id, 2L) |> 
  ...
  mutate(delta_x = row_2 - row_1,
         ...
         diago = sign(delta_x) == sign(delta_y)) |>  
  rowwise() |> 
  mutate(row_anti1 = pmap(list(d = diago, r1 = row_1, r2 = row_2, dx = delta_x), row_anti),
         col_anti1 = pmap(list(d = diago, c1 = col_1, c2 = col_2, dy = delta_y), col_anti))
